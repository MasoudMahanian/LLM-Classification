...
classifier = TransformerClassifier(classes=classes)
device = classifier.device
# بعد از بارگذاری مدل، دستگاه رو چک کن

def prepare_dataloader(texts, labels, agent, batch_size=32, max_length=classifier.max_len):
...
for epoch in range(EPOCHS):
    ...
    for batch in progress_bar:
        ...
        # quit()
        outputs = classifier.model(input_ids)
...
plt.show()
plt.tight_layout()
plt.savefig("Transformer_classifier_training_curves.png", dpi=300, bbox_inches='tight')
plt.close()  

def save_model(model, filepath="Transformer_classifier.pth"):

    torch.save(model.state_dict(), filepath)
    print(f"✅ model saved {filepath} !")

save_model(classifier.model, "Transformer_classifier.pth")